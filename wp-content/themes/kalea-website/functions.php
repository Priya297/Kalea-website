<?php
/**
 * kaleawebsite functions and definitions
 *
 ...
 * @since kaleawebsite 1.0
 */
function kaleawebsite_setup() {
	/*
	 * Makes kaleawebsite available for translation.
	 *
	 * Translations can be filed at WordPress.org. See: https://translate.wordpress.org/projects/wp-themes/kaleawebsite
	 * If you're building a theme based on kaleawebsite, use a find and replace
	 * to change 'kaleawebsite' to the name of your theme in all the template files.
	 */
	load_theme_textdomain( 'kaleawebsite' );

	// This theme styles the visual editor with editor-style.css to match the theme style.
	add_editor_style( array( 'assets/css/admin/editor-style.css', kaleawebsite_fonts_url() ) );


	/**
	 * Adding Primary & Footer menu location for wp_nav_menu
	 */
	register_nav_menu( 'primary', __( 'Primary Menu', 'kaleawebsite' ) );
	register_nav_menu( 'footer', __( 'Footer Menu', 'kaleawebsite' ) );
...
add_action( 'after_setup_theme', 'kaleawebsite_setup' );

/**
 * Register custom fonts.
 */
function kaleawebsite_fonts_url() {
	$fonts_url = '';
 ...
 * @since kaleawebsite 1.0
 */
function kaleawebsite_scripts_styles() {
	global $wp_styles;
	...
	//Enqueue CSS Files
	wp_enqueue_style( 'fontawersome-min', CSS_PATH.'fontawersome-min.css' );
	wp_enqueue_style( 'magnific-popup', CSS_PATH.'magnific-popup.css' );
	wp_enqueue_style( 'mcustomscrollbar-min', CSS_PATH.'mcustomscrollbar.css' );
	wp_enqueue_style( 'slick', CSS_PATH.'slick.css' );
	

	//Enqueue JS Files
	wp_enqueue_script( 'slick-js', JS_PATH . 'slick.js', array(), $theme_version, true );
	wp_enqueue_script( 'magnific-popup', JS_PATH . 'jquery.magnific-popup.min.js', array(), $theme_version, true );
	wp_enqueue_script( 'mcustomscrollbar', JS_PATH . 'mcustomscrollbar.js', array(), $theme_version, true );
	wp_enqueue_script( 'lazy', JS_PATH . 'jquery.lazy.min.js', array(), $theme_version, true  );
	wp_enqueue_script( 'lazy.plugins', JS_PATH . 'jquery.lazy.plugins.min.js', array(), $theme_version, true  );
	wp_enqueue_script( 'theme-main-js', JS_PATH.'theme-main.js', array(), $theme_version, true );
	wp_enqueue_script( 'general-js', JS_PATH.'general.js', array(), $theme_version, true );
	wp_enqueue_script( 'custom-js', JS_PATH.'custom.js', array(), $theme_version, true );
	wp_enqueue_style( 'kaleawebsite-ie', CSS_PATH.'admin/ie.css' );
	if ( ( ! is_admin() ) && is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
	$wp_styles->add_data( 'kaleawebsite-ie', 'conditional', 'lt IE 9' );
	
	// Loads our main stylesheet.
	wp_enqueue_style( 'kaleawebsite-style', get_stylesheet_uri() );

	wp_enqueue_style( 'responsive', CSS_PATH.'responsive.css' );
}
add_action( 'wp_enqueue_scripts', 'kaleawebsite_scripts_styles' );
 ...
 *
 * @since kaleawebsite 2.2
 *
 * @param array   $urls          URLs to print for resource hints.
 * @param string  $relation_type The relation type the URLs are printed.
 * @return array URLs to print for resource hints.
 */
function kaleawebsite_resource_hints( $urls, $relation_type ) {
	if ( wp_style_is( 'kaleawebsite-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
...
add_filter( 'wp_resource_hints', 'kaleawebsite_resource_hints', 10, 2 );
 ...
 */
function kaleawebsite_wp_title( $title, $sep ) {
	global $paged, $page;
	...
	if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() )
		$title = "$title $sep " . sprintf( __( 'Page %s', 'kaleawebsite' ), max( $paged, $page ) );

	return $title;
}
add_filter( 'wp_title', 'kaleawebsite_wp_title', 10, 2 );
 ...
 * @since kaleawebsite 1.0
 */
function kaleawebsite_page_menu_args( $args ) {
	if ( ! isset( $args['show_home'] ) )
		$args['show_home'] = true;
	return $args;
}
add_filter( 'wp_page_menu_args', 'kaleawebsite_page_menu_args' );
 ...
 * @since kaleawebsite 1.0
 */
function kaleawebsite_widgets_init() {
	register_sidebar( array(
		'name' => __( 'Main Sidebar', 'kaleawebsite' ),
		'id' => 'sidebar-1',
		'description' => __( 'Appears on posts and pages except the optional Front Page template, which has its own widgets', 'kaleawebsite' ),
	...
	register_sidebar( array(
		'name' => __( 'First Front Page Widget Area', 'kaleawebsite' ),
		'id' => 'sidebar-2',
		'description' => __( 'Appears when using the optional Front Page template with a page set as Static Front Page', 'kaleawebsite' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
	...
	register_sidebar( array(
		'name' => __( 'Second Front Page Widget Area', 'kaleawebsite' ),
		'id' => 'sidebar-3',
		'description' => __( 'Appears when using the optional Front Page template with a page set as Static Front Page', 'kaleawebsite' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget' => '</aside>',
	...
	if( is_plugin_active( ACF_PLUGIN_CHECK ) ):
		if(options('show_footer_widget_area') == 1) :
			...
			for($i=1;$i<=$count;$i++)
			{
				...
				$w_id = 'footer-'.$i;
				register_sidebar( array(
					'name' => __( $w_name, 'kaleawebsite' ),
					'id' => $w_id ,
					'description' => __( 'Appears when using Footer widget Enables', 'kaleawebsite' ),
					'before_widget' => '<aside id="%1$s" class="widget %2$s">',
					'after_widget' => '</aside>',
					'before_title' => '<h3 class="widget-title">',
					'after_title' => '</h3>',
...
add_action( 'widgets_init', 'kaleawebsite_widgets_init' );


if ( ! function_exists( 'kaleawebsite_content_nav' ) ) :
/**
 * Displays navigation to next/previous pages when applicable.
 *
 * @since kaleawebsite 1.0
 */
function kaleawebsite_content_nav( $html_id ) {
	global $wp_query;

	if ( $wp_query->max_num_pages > 1 ) : ?>
		<nav id="<?php echo esc_attr( $html_id ); ?>" class="navigation" role="navigation">
			<h3 class="assistive-text"><?php _e( 'Post navigation', 'kaleawebsite' ); ?></h3>
			<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'kaleawebsite' ) ); ?></div>
			<div class="nav-next"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'kaleawebsite' ) ); ?></div>
		</nav><!-- .navigation -->
	<?php endif;
}
endif;

if ( ! function_exists( 'kaleawebsite_comment' ) ) :
/**
 * Template for comments and pingbacks.
 *
 * To override this walker in a child theme without modifying the comments template
 * simply create your own kaleawebsite_comment(), and that function will be used instead.
 *
 * Used as a callback by wp_list_comments() for displaying the comments.
 *
 * @since kaleawebsite 1.0
 */
function kaleawebsite_comment( $comment, $args, $depth ) {
	...
	<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
		<p><?php _e( 'Pingback:', 'kaleawebsite' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( '(Edit)', 'kaleawebsite' ), '<span class="edit-link">', '</span>' ); ?></p>
	<?php
			break;
		default :
			...
			<header class="comment-meta comment-author vcard">
				<?php
					...
					printf( '<cite><b class="fn">%1$s</b> %2$s</cite>',
						...
						( $comment->user_id === $post->post_author ) ? '<span>' . __( 'Post author', 'kaleawebsite' ) . '</span>' : ''
					);
					printf( '<a href="%1$s"><time datetime="%2$s">%3$s</time></a>',
						esc_url( get_comment_link( $comment->comment_ID ) ),
						get_comment_time( 'c' ),
						/* translators: 1: date, 2: time */
						sprintf( __( '%1$s at %2$s', 'kaleawebsite' ), get_comment_date(), get_comment_time() )
					);
				?>
			</header><!-- .comment-meta -->

			<?php if ( '0' == $comment->comment_approved ) : ?>
				<p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'kaleawebsite' ); ?></p>
			<?php endif; ?>

			<section class="comment-content comment">
				<?php comment_text(); ?>
				<?php edit_comment_link( __( 'Edit', 'kaleawebsite' ), '<p class="edit-link">', '</p>' ); ?>
			</section><!-- .comment-content -->

			<div class="reply">
				<?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( 'Reply', 'kaleawebsite' ), 'after' => ' <span>&darr;</span>', 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
			</div><!-- .reply -->
		</article><!-- #comment-## -->
	<?php
		break;
...
if ( ! function_exists( 'kaleawebsite_entry_meta' ) ) :
/**
 * Set up post entry meta.
 *
 * Prints HTML with meta information for current post: categories, tags, permalink, author, and date.
 *
 * Create your own kaleawebsite_entry_meta() to override in a child theme.
 *
 * @since kaleawebsite 1.0
 */
function kaleawebsite_entry_meta() {
	// Translators: used between list items, there is a space after the comma.
	$categories_list = get_the_category_list( __( ', ', 'kaleawebsite' ) );

	// Translators: used between list items, there is a space after the comma.
	$tag_list = get_the_tag_list( '', __( ', ', 'kaleawebsite' ) );
	...
	$author = sprintf( '<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>',
		...
		esc_attr( sprintf( __( 'View all posts by %s', 'kaleawebsite' ), get_the_author() ) ),
		get_the_author()
	);

	// Translators: 1 is category, 2 is tag, 3 is the date and 4 is the author's name.
	if ( $tag_list ) {
		$utility_text = __( 'This entry was posted in %1$s and tagged %2$s on %3$s<span class="by-author"> by %4$s</span>.', 'kaleawebsite' );
	} elseif ( $categories_list ) {
		$utility_text = __( 'This entry was posted in %1$s on %3$s<span class="by-author"> by %4$s</span>.', 'kaleawebsite' );
	} else {
		$utility_text = __( 'This entry was posted on %3$s<span class="by-author"> by %4$s</span>.', 'kaleawebsite' );
	}
 ...
 */
function kaleawebsite_body_class( $classes ) {
	...
	if ( wp_style_is( 'kaleawebsite-fonts', 'queue' ) )
...
add_filter( 'body_class', 'kaleawebsite_body_class' );
 ...
 * @since kaleawebsite 1.0
 */
function kaleawebsite_content_width() {
	if ( is_page_template( 'page-templates/full-width.php' ) || is_attachment() || ! is_active_sidebar( 'sidebar-1' ) ) {
		global $content_width;
		$content_width = 960;
	}
}
add_action( 'template_redirect', 'kaleawebsite_content_width' );
 ...
 */
function kaleawebsite_customize_register( $wp_customize ) {
	...
	if ( isset( $wp_customize->selective_refresh ) ) {
		$wp_customize->selective_refresh->add_partial( 'blogname', array(
			...
			'render_callback' => 'kaleawebsite_customize_partial_blogname',
		) );
		$wp_customize->selective_refresh->add_partial( 'blogdescription', array(
			'selector' => '.site-description',
			'container_inclusive' => false,
			'render_callback' => 'kaleawebsite_customize_partial_blogdescription',
		) );
	}
}
add_action( 'customize_register', 'kaleawebsite_customize_register' );

/**
 * Render the site title for the selective refresh partial.
 *
 * @since kaleawebsite 2.0
 * @see kaleawebsite_customize_register()
 *
 * @return void
 */
function kaleawebsite_customize_partial_blogname() {
	bloginfo( 'name' );
}
 ...
 * @return void
 */
function kaleawebsite_customize_partial_blogdescription() {
 ...
 * @since kaleawebsite 1.0
 */
function kaleawebsite_customize_preview_js() {
	wp_enqueue_script( 'kaleawebsite-customizer', JS_PATH.'theme-customizer.js', array( 'customize-preview' ), '20141120', true );
}
//add_action( 'customize_preview_init', 'kaleawebsite_customize_preview_js' );
 ...
 */
function kaleawebsite_mime_types($mimes) {
  $mimes['svg'] = 'image/svg+xml';
  return $mimes;
}
add_filter('upload_mimes', 'kaleawebsite_mime_types');
	...
	function social_list() {			
		...
		if( have_rows('social_links', 'option') ):
			...
			while ( have_rows('social_links', 'option') ) : the_row();
				...
				$html .= '<li><a href="'.$mediaLink.'" target="_blank"><i class="'.$mediaClass.'"></i></a></li>';
			endwhile;
			$html .= '</ul>';
		endif;
	...
	if( options('website_under_maintenance') == 1 ):
		...
		function wp_maintenance_mode(){
			if(!current_user_can('edit_themes') || !is_user_logged_in()){
			    wp_dequeue_style( 'kaleawebsite-style' );
