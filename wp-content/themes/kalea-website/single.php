...
<!-- breadcrumb section -->
	...
	while(have_rows('flexible_content')): the_row();
		if( get_row_layout() == 'banner_section'):
			...
			$banner_cover_class = get_sub_field('banner_cover_class');
			$banner_overlay = get_sub_field('banner_overlay');
			$title 		 =  get_sub_field('title');
			$banner_highlight = get_sub_field('banner_highlight');
			$content 	 =  get_sub_field('content');
			$detail_link =  get_sub_field('detail_link');
			$link_target = $detail_link['target'] ? $detail_link['target'] : '_self';
?>	
<!-- inner banner 2 -->
<!-- Note : Add class banner-overlay for add overley -->
<section class="inner-page-banner <?php if($banner_overlay == true){ echo 'banner-overlay'; } ?>">
	<div class="banner-back <?php if(!empty($banner_cover_class)){ echo $banner_cover_class; }?>" style="background-image: url('<?php echo $banner_background_image['url'];?>');	">
	</div>
	<!-- Note : add highlited-text class if you want title in blue and paragraph in black -->
	<div class="container">
		<div class="banner-content <?php if(!empty($banner_highlight)){ echo $banner_highlight; }?>">
...
elseif( get_row_layout() == 'content_section' ):
	...
	$display_autordate 	=	get_sub_field('display_autordate');
if(!empty($content) || !empty($side_image) || !empty($bottom_content) || !empty($display_autordate)): 
?>
				...
				$author = get_the_author();  ?>
					<div class="blog-author-name">
		      		...
		      		<div class="blog-post-date">
		      			<strong>Date: </strong><?php echo get_the_date('d-m-y');?>
		      		</div>
		      	<?php endwhile;
		      endif; } ?>
			<?php echo $content; ?>
...
<div class="new-treatment-section">
	<div class="new-treatment-inner">
		<div class="container">
			...
			<p><?php echo $content; ?></p>
			<?php if(!empty($detail_link)){ ?>
				<a href="#" title="Boek een vrijblijvend adviesgesprek" class="transperent-btn-blue">Boek een vrijblijvend adviesgesprek</a>
			<?php } ?>
		</div>
	</div>
</div>
	...
	function my_repeater_show_more() {
		...
		jQuery.post(
			...
			function (json) {
				...
				jQuery('.review-inner').append(json['content']);
				// update offset
